...
- **GLM 5.3 on Cloudflare Workers AI**: Added `glm-5.3-cf` for Cloudflare's `@cf/zai-org/glm-5.3`, with reasoning and structured-output metadata, registry coverage, and a dedicated live completion smoke test (`01dca81`).
- **GLM 5.3 Flash on OpenRouter**: Added the multimodal `glm-5.3-flash-openrouter` profile with structured output, `low`/`high`/`max` reasoning effort, vision support, baseline pricing metadata, and regression coverage (`af208f9`).

## 0.0.148 · 2026-08-26

- **Expanded Cloudflare Workers AI catalog**: Added Cloudflare-hosted profiles for GLM 5.3 Flash, Qwen 3.8 27B, DeepSeek V4 Pro and Flash, Moondream 3.1, GLM 5.2, Kimi K2.7 Code, and Kimi K2.6. The new entries expose the documented reasoning, vision, and structured-output capabilities through `-cf` model IDs (`4acf72a`).
- **Cloudflare Moondream support**: Added a model-specific `/ai/run` adapter for Moondream 3.1, including image-to-text request mapping and recursive Cloudflare response-envelope normalization. Registry, request, response, and live completion smoke tests cover all eight new Cloudflare models (`4acf72a`).
- **GLM 5.3 reasoning metadata**: Preserved OpenRouter's default reasoning effort while adding explicit `max` effort support and regression coverage (`27551e8`).

## 0.0.147 · 2026-08-22

- **MCP client coroutine-safe**: Make MCP client coroutine-safe (`53cd28c`).
- **GLM 5.3 on OpenRouter**: Added `glm-5.3-openrouter` (`z-ai/glm-5.3`) with pricing ($1.40/M input, $4.40/M output, $0.26 cached read) (`1ad77e2`).

## 0.0.146 · 2026-08-11

- **Observable Responses agent loop**: `start()` and `start_nowait()` accept `max_rounds` (replacing the hardcoded 100-round limit, validated `>= 1`) and an `on_message` async callback that receives each `Message` as it is appended to the loop's working conversation — the assistant message before its local tools execute, the tool message after that round's tools finish. `APIResponse` gains additive `trajectory` (the full multi-round `Conversation`, previously discarded) and `loop_stop_reason` (`"no_tool_calls"`, `"max_rounds"`, or `"error"`) fields. Trajectories preserve input `Message.extra` and `model_used`, and record the responding model per round. When `max_rounds` is reached with pending tool calls, the calls are no longer executed (their results could never be consumed). The `run_agent_loop` Responses-mode guard is unchanged. See `AGENT_LOOP_STREAMING_SPEC.md`.
- **Lossless conversation logs (v2)**: `to_log()` now defaults to `lossless=True`, preserving full tool results (including list-of-parts results that previously became `"<Tool result (N blocks)>"` placeholders), `Thinking.raw_payload`/`id`/`summary` (encrypted Responses reasoning items), `ToolCall.built_in`/`built_in_type`/`extra_body`, `ToolResult` metadata and files, `Message.extra`, and media bytes. A lossless round-trip reproduces identical `to_openai_responses()` input items. `lossless=False` opts into compact logs that drop only large opaque payloads (media bytes, raw reasoning, raw response items) with typed omission markers. `preserve_media` is deprecated but honored (`True` → lossless, `False` → compact) with a `DeprecationWarning`. `from_log()` loads v1 logs, v2 lossless, and v2 compact payloads best-effort without raising. `Conversation.to_log`/`from_log` now delegate to the `Message`-level serializers instead of maintaining a second copy. See `LOG_V2_SPEC.md`.

## 0.0.145 · 2026-08-11

- **Meta Muse Spark models**: Replaced the retired direct Meta Llama catalog with `muse-spark-1.1`, `muse-spark-1.2`, and `muse-spark-1.2-contributor` through the Meta Model API. The new profiles support images, structured output, reasoning, Chat Completions, and the Responses API. Meta credentials can now come from either `META_API_KEY` or `MODEL_API_KEY`, with `META_API_KEY` taking precedence. Documentation calls out the contributor model's training-data terms.
- **Stateless Responses history**: Added `stateless_responses` to `LLMClient` and request contexts. Stateless requests set `store=false`, request encrypted reasoning content, and replay that reasoning safely in later turns; validation rejects incompatible stored-history and background-mode combinations. Models can declare a preferred or required history mode, and Muse Spark requires stateless operation. Responses serialization now preserves encrypted reasoning even without tool calls and emits a valid follower item when needed.
- **Expanded model registry**: Added OpenRouter profiles for DeepSeek V4 Flash 0731, Inkling and Inkling Small, Laguna S/XS 2.1, Kimi K2.7 Code, Kimi K3, Muse Spark 1.1, GLM 5.2, Nemotron 3 Ultra, and MiniMax M3. Registry initialization now consistently treats mapping keys as canonical IDs, fixing a copy-paste collision that made `nemotron-3-ultra-openrouter` unreachable.
- **Agent-loop streaming design**: Added an implementation-ready specification for a unified observable agent loop with streamed text, reasoning, tool-call lifecycle events, cancellation semantics, compatibility guarantees, and a staged migration plan.

## 0.0.144 · 2026-07-25

- **GPT-5.6 models**: Added OpenAI `gpt-5.6-sol`, `gpt-5.6-terra`, and `gpt-5.6-luna`, with the `gpt-5.6` alias routing to Sol. GPT-5.6 supports Responses API generation, image input, structured outputs, `xhigh` and `max` reasoning efforts, explicit `reasoning_effort="none"`, verbosity, and cache-write-aware pricing metadata. Responses requests now merge `extra_body["reasoning"]`, enabling small passthroughs such as `{ "mode": "pro" }` without adding a separate client option. Reasoning-effort normalization now uses model capability metadata, accepts `max` throughout the public client and proxy types, safely maps unsupported values for older OpenAI-compatible, Gemini, and Anthropic models, and preserves canonical model IDs such as `gpt-5.1-codex-max` ahead of suffix parsing. Live validation passed for Sol, Terra, and Luna through the `lm-deluge` 1Password environment.
- **Claude Opus 5**: Added native Anthropic `claude-5-opus` support plus `claude-5-opus-bedrock` and `claude-5-opus-bedrock-global` profiles. Opus 5 uses adaptive summarized thinking, `xhigh`/`max` effort, task budgets, sampling-parameter stripping, and explicit disabled-thinking handling for `reasoning_effort="none"` and `thinking_budget=0`. Added focused request tests and live Anthropic/Bedrock region-audit scripts.
- **Bedrock reasoning fixes**: Bedrock Opus 4.5 no longer receives the unsupported GA `output_config` field, while newer models retain shared effort handling. Summarized thinking from Claude 4.7+ is now preserved as a summary with its signed raw payload shaped for safe multi-turn round trips. Expanded unit and live coverage across disabled, adaptive, and budget-based thinking modes.
- **AWS Lambda MicroVM sandbox add-on**: Added the opt-in `LambdaMicroVMSandbox` and content-addressed `LambdaMicroVMImageBuilder`, including Docker-context filtering with pre-read size/type enforcement, image reuse/rebuild handling, idempotent launches, authenticated command-agent access, cancellation-safe cleanup, suspend/resume support, bounded command-output reads, fail-closed egress configuration, documentation, and unit/live smoke tests.
- **Dependency and build hardening**: Raised minimum-safe Python dependency versions across core and optional extras, including aiohttp, Pillow, Pygments, AWS, Google, Docker, and server dependency trees, and aligned the legacy `requirements.txt` with package metadata. Migrated the documentation site and Cloudflare sandbox worker from npm lockfiles to pnpm, upgraded vulnerable Astro/Wrangler dependency graphs, pinned the Cloudflare Sandbox SDK, enforced Node 22 floors, made the worker deploy script install from its frozen pnpm lockfile, and updated preview routing for the pinned Sandbox SDK API.

## 0.0.143 · 2026-07-08

- **Bedrock now forwards thinking/effort settings**: The Bedrock Anthropic request builder previously ignored `thinking_budget`, `reasoning_effort`, and `global_effort` entirely — thinking could never be enabled via Bedrock, silently. The reasoning/effort logic is now shared with the direct Anthropic builder (`apply_anthropic_reasoning_config`), so both routes send identical `thinking` and `output_config` payloads: adaptive thinking + effort on Claude 4.6+, `budget_tokens` (with the `max_tokens` bump and temperature pinning) on older reasoning models, and effort-to-budget translation. Models without extended thinking omit the `thinking` field, and Claude 4.6 on Bedrock drops non-default `temperature` when thinking is active, mirroring the direct route.
- **Behavior change**: Claude 4.6+ Bedrock models now default to adaptive thinking with `output_config.effort: high`, matching the direct Anthropic API. Adaptive thinking shares the `max_new_tokens` budget with visible output (by design — adaptive usage can't be predicted, so headroom is the caller's responsibility). Callers relying on the old "Bedrock never thinks" behavior should pass `reasoning_effort="none"` or raise `max_new_tokens`.
- **Bedrock-aware model version checks**: `_is_claude_47`, `_is_claude_5_sonnet`, and GA-effort detection now recognize Bedrock model names (e.g. `us.anthropic.claude-fable-5`, `us.anthropic.claude-sonnet-5`), so version-gated request features apply consistently across routes.

## 0.0.142 · 2026-07-02

- **Claude Sonnet 5**: Added `claude-5-sonnet` for the Anthropic API with `claude-sonnet-5` alias support, adaptive thinking defaults, removed manual thinking-budget translation, sampling-parameter stripping, assistant-prefill blocking, and focused request/live coverage. Added Bedrock profiles `claude-5-sonnet-bedrock` (`us.anthropic.claude-sonnet-5`) and `claude-5-sonnet-bedrock-global` (`global.anthropic.claude-sonnet-5`). Live validation passed through `bop run lm-deluge -- ...` for the Anthropic API, all 6 configured US Bedrock source regions, and all 30 configured global Bedrock source regions.
- **Invalid reasoning config validation**: Added request validation that fails fast for incompatible model/request combinations instead of sending requests that the provider rejects later. Anthropic adaptive-thinking requests now strip or reject non-default `temperature`, and GPT-5.4/GPT-5.5 reasoning models now reject function tools on Chat Completions with a clear `use_responses_api=True` error.

## 0.0.141 · 2026-06-12

- **Gemini 3.1 Flash-Lite GA**: Added `gemini-3.1-flash-lite` for the native Gemini API and updated `gemini-3.1-flash-lite-compat` to target the GA model name instead of the discontinued preview. The preview ID remains registered for backward compatibility. Live validation passed for native basic completion, native tool use, and the OpenAI-compatible basic completion path.
...
- **JustBashSandbox**: New cross-platform sandbox powered by [Vercel's just-bash](https://github.com/vercel-labs/just-bash). Provides read-scoped, network-disabled, copy-on-write execution without Docker or platform-specific tooling. Supports configurable `root_dir`, `working_dir`, read-only mode, optional Python via `enable_python`, auto-install of the `just-bash` CLI, background process tracking, and the same `bash` / `list_processes` tool interface as other sandboxes. See [Tool Use](/features/tools/) for usage.
...
- **PybubbleSandbox**: New Linux sandbox backed by [pybubble](https://github.com/nichochar/pybubble) (bubblewrap). Provides filesystem and process isolation without Docker — just needs `bwrap` installed. Supports configurable network access (`network_access`, `outbound_access`, `allow_host_loopback`), optional fallback to host-network sharing in restricted runtimes (requires `allow_host_loopback=True`), background process tracking, and the same `bash` / `list_processes` tool interface as other sandboxes. Requires the `pybubble` dependency and Linux. See [Tool Use](/features/tools/) for usage.
...
- **New `SqliteManager` prefab tool**: Schema-first SQLite tool for agents — supports `list_tables`, `describe_table`, and `query` commands with progressive disclosure. Build a DB from a list-of-dicts via `SqliteManager.from_dicts()` (auto-infers column types including JSON columns) or point at an existing `.db` file. Supports read-only mode, parameterized queries, multiple output formats (JSON, YAML, CSV, TSV), row-count truncation, and sample rows. See [Tool Use](/features/tools/) for usage.
...
- **New docs page**: Added [Embeddings](/features/embeddings/) documentation with model table, examples, and configuration reference.
