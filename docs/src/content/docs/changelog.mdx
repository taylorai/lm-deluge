...
- Added `FilesystemManager`, an in-memory virtual workspace + tool wrapper that gives agents sandboxed `read_file` / `write_file` / `list_dir` / `grep` / `apply_patch` capabilities without touching the host filesystem; the implementation lives in `lm_deluge.llm_tools.filesystem`.
