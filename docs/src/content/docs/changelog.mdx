...
- Prefab expansion: added Google Docs tools (metadata, ranged reads/grep, markdown-aware insert/update/replace/delete) plus a Google Sheets manager (list tabs, find used ranges, read ranges as HTML tables, update cells) with service-account auth.
- Utility helpers: Exa-powered `WebSearchManager` (search + URL fetch), AWS SES `EmailManager` with recipient allowlists and optional HTML/CC/BCC, and `RandomTools` for floats/ints/choices/secure tokens (with a new example).
- Storage upgrades: FilesystemManager gained an S3-backed workspace with optimistic locking and the full read/write/list/grep/apply_patch surface, while MemoryManager adds an S3-backed scratchpad with conditional writes/retries for shared agents.
...
- Pipelines split: `extract`, `translate`, and `score_llm` now live in `lm_deluge.pipelines` with backward exports from `lm_deluge.llm_tools`; the latter now only re-exports pipelines/prefabs for compatibility.
