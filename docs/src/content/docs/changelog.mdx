...
- **PybubbleSandbox**: New Linux sandbox backed by [pybubble](https://github.com/nichochar/pybubble) (bubblewrap). Provides filesystem and process isolation without Docker — just needs `bwrap` installed. Supports configurable network access (`network_access`, `outbound_access`, `allow_host_loopback`), automatic fallback to host-network sharing in restricted runtimes (e.g. Modal), background process tracking, and the same `bash` / `list_processes` tool interface as other sandboxes. Requires the `pybubble` dependency and Linux. See [Tool Use](/features/tools/) for usage.
...
- **Agent loop default prompt caching**: `run_agent_loop` now defaults to `cache="last_3_user_messages"` when no cache pattern is specified. On Anthropic (and other providers that support prompt caching), this avoids paying full price for the growing conversation prefix on every round. On providers that don't support caching the parameter is silently ignored.
