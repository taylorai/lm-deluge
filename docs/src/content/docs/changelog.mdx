...
- **Audio transcription**: New `lm_deluge.transcribe` module — a lightweight, standalone transcription client (like `embed`). Supports OpenAI (`whisper-1`, `gpt-4o-transcribe`, `gpt-4o-mini-transcribe`), Mistral (`voxtral-mini-latest`), and Fireworks (`whisper-v3`, `whisper-v3-turbo`). Parallel batch transcription with rate limiting, retries, cost tracking, and progress bars. Use `transcribe_async` / `transcribe_sync` with file paths, `Path` objects, or raw bytes.
