...
- **Bedrock-aware model version checks**: `_is_claude_47`, `_is_claude_5_sonnet`, and GA-effort detection now recognize Bedrock model names (e.g. `us.anthropic.claude-fable-5`, `us.anthropic.claude-sonnet-5`), so version-gated request features apply consistently across routes. Exception: Bedrock Opus 4.5 rejects `output_config` ("Extra inputs are not permitted", verified live), so GA effort on 4.5 remains direct-API only.
- **Live validation**: Verified against real Bedrock endpoints — adaptive thinking + `output_config.effort` accepted on 4.6/4.7/4.8/Sonnet 5/Fable 5, `budget_tokens` on 4.5 Sonnet returns thinking content, explicit `thinking: disabled` accepted on 4.1/4.5/4.6.
