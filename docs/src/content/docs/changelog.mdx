...
- Proxy server adds configurable model policy (allowlists, defaults, alias routes) with CLI/config support, plus optional request/provider logging and forwarded `anthropic-beta` headers.
...
- Added `minimax-m2.1` via OpenRouter, expanding the model catalog.
